...
async def get_overview(db: AsyncSession) -> dict:
    ...
    in_progress = (
        await db.execute(select(func.count(Task.id)).where(Task.status == TaskStatus.IN_PROGRESS))
    ).scalar()
    overdue = (
        await db.execute(
            select(func.count(Task.id)).where(Task.status != TaskStatus.DONE, Task.deadline < now)
        )
    ).scalar()
    done = (await db.execute(select(func.count(Task.id)).where(Task.status == TaskStatus.DONE))).scalar()
...
async def get_team_workload(db: AsyncSession) -> list[dict]:
    ...
    for u in users:
        assigned = (
            await db.execute(select(func.count(Task.id)).where(Task.assignee_id == u.id, Task.status != TaskStatus.DONE))
        ).scalar()
        in_prog = (
            await db.execute(select(func.count(Task.id)).where(Task.assignee_id == u.id, Task.status == TaskStatus.IN_PROGRESS))
        ).scalar()
        completed = (
            await db.execute(select(func.count(Task.id)).where(Task.assignee_id == u.id, Task.status == TaskStatus.DONE))
        ).scalar()
        overdue = (
            await db.execute(
                select(func.count(Task.id)).where(Task.assignee_id == u.id, Task.status != TaskStatus.DONE, Task.deadline < now)
            )
        ).scalar()
        result.append({
